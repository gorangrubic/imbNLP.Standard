...
namespace System.Grams
{
    public class AppGloVe
    {
        ...
        Options ParseParams(string[] args)
        {
            Options Latin()
            {
                return new Options()
        ...
        unsafe void Train(Options options, IOrthography lang, Hash model, int VECTOR)
        {
            Hash digrams = null;
        ...
        unsafe void Analogy(IOrthography lang, int VECTOR, Hash space)
        {
            Gram[] W = space.List();
            Console.Write("\r\n");
            while (true)
        ...
        unsafe void Graph(IOrthography lang, int VECTOR, Gram[] W)
        {
        ...
        void Norm(IEnumerable<Gram> grams)
        {
        ...
        unsafe (string key, float distance)[] Find(Gram[] W, Gram q, Func<string, bool> bans)
        {
        ...
        unsafe void Shuffle(Gram[] shuffle)
        {
            Random r = new Random();
            for (int i = 0; i < shuffle.Length; i++)
            {
